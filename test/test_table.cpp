...
public:
    TestTable(Allocator& a) : Table(a) { init(); }
    TestTable() { init(); }
    void init()
    {
    ...
    void add(int first, int second, bool third, Days forth)
    {
        size_t ndx = add_empty_row(1);
        set_int(0, ndx, first);
        set_int(1, ndx, second);
        set_bool(2, ndx, third);
        set_int(3, ndx, forth);
    }
};
...
class TestTableEnum : public Table {
public:
    TestTableEnum()
    {
        add_column(type_Int, "first");
    ...
    void add(Days first, std::string second)
    {
        size_t ndx = add_empty_row(1);
        set_int(0, ndx, first);
        set_string(1, ndx, second);
    }
};
...
class TestTableFloats : public Table {
public:
    TestTableFloats()
    {
    ...
    void add(float first, double second)
    {
        size_t ndx = add_empty_row(1);
        set_float(0, ndx, first);
        set_double(1, ndx, second);
    }
};
...
class TestTableAE : public Table {
public:
    TestTableAE()
    {
        add_column(type_Int, "first");
    ...
    void add(int first, std::string second, bool third, Days forth)
    {
        size_t ndx = add_empty_row(1);
        set_int(0, ndx, first);
        set_string(1, ndx, second);
...
namespace {
REALM_TABLE_1(TestTableMX, first, Mixed)
} // anonymous namespace
...
namespace {
REALM_TABLE_2(MyTable1, val, Int, val2, Int)

REALM_TABLE_2(MyTable2, val, Int, subtab, Subtable<MyTable1>)

REALM_TABLE_1(MyTable3, subtab, Subtable<MyTable2>)

REALM_TABLE_1(MyTable4, mix, Mixed)
} // anonymous namespace
...
namespace {
REALM_TABLE_2(TableDateAndBinary, date, OldDateTime, bin, Binary)
} // anonymous namespace
...
namespace {
REALM_TABLE_3(TableAgg, c_int, Int, c_float, Float, c_double, Double)
...
namespace {
REALM_TABLE_1(TableAgg2, c_count, Int)
} // anonymous namespace
...
REALM_TABLE_3(TablePivotAgg, sex, String, age, Int, hired, Bool)
